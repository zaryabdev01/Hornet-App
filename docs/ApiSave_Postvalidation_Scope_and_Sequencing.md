---
title: "ApiSave — Post-Validation Work: Scope, Effort and Sequencing"
...
You have approved the direction for all three post-validation items. This document gives
the effort estimate, the recommended sequence, and a recommendation on whether each belongs
inside M3.

Rate basis: **$250 / day**, as used elsewhere in this engagement. Dollar figures are derived
from the day ranges; the day ranges are the real estimate.
...
**Effort: 3–5 days** (~$750–$1,250).

- ~1 day — implement the five changes, synthetic Judge tests.
- ~1.5–3 days — iterative live regression. Prompt tuning is volatile (M2 took four rounds);
  the baseline run on day one will show where in this range it lands.
- ~0.5–1 day — report with raw decision traces, before/after evidence, `.docx`.

**Inside M3? — No, recommended separate.** M3's headline metric is the native-valid JSON
rate. Changing the prompt and schema mid-M3 makes that number impossible to attribute — the
same reason the photo-#10 experiment was scoped outside M3. Practically, this work bumps the
prompt and schema versions, and M3's atomic protocol bundle then simply references the
corrected versions. It should be done **before** M3 so M3 validates against the corrected
baseline.
...
**Effort: 2–3 days** (~$500–$750).

**Inside M3? — No, independent.** It can run in parallel with Item 1 or immediately after.
It gives the client immediate relief from the 503s being seen now, rather than waiting on
M3.
...
**Nature:** new advisory feature. No verdict ever changes.
...
**Effort: 1.5–2.5 days** (~$375–$625).

**Inside M3? — No, independent.** Product feature, unrelated to M3 infrastructure. Lowest
risk of the three; can slot in wherever convenient.
...
|---|---|---|---|
| 1 | **Item 1** — false-negative correction | 3–5 | Safety-critical (false negative on the target species). Detection baseline must be stable before any further build ships or M3 validates against it. |
| 2 | **Item 2** — 503 / latency | 2–3 | Immediate user-visible pain. Independent of Item 1; can overlap its validation waits. |
| 3 | **Item 3** — distant-structure advisory | 1.5–2.5 | Lowest urgency and risk. Can run in parallel with Item 2. |
| — | **M3** | (its own scope) | Proceeds separately once you confirm the hosting platform. Built on the Item 1 corrected prompt/schema. |

**Combined for the three items: 6.5–10.5 days (~$1,625–$2,625).** If you would rather treat
them as one approval and one invoice rather than three, that is fine — the sequence and
...
1. Confirm the effort range and sequence above, or tell me what to adjust.
2. Confirm you are happy for the three items to be billed **separately from M3** (recommended),
   or bundled — your call.
3. The hosting platform for M3, when you have it (you said you would send this separately).
